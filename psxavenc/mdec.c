...
void encode_frame_bs(uint8_t *video_frame, settings_t *settings)
{
			...
			settings->state_vid.quant_scale++)
	{
		...
		if (!encode_bits(&(settings->state_vid), 10, 0x1FF))
		{
			continue;
		}
		if (!encode_bits(&(settings->state_vid), 2, 0x2))
		{
			continue;
		}
		if (!flush_bits(&(settings->state_vid)))
		{
			continue;
		}
	...
	if (settings->state_vid.blocks_used > 65535)
	{
		fprintf(stderr, "Error: blocks_used (%d) exceeds 65535; MDEC data is too large!\n", settings->state_vid.blocks_used);
		exit(1);
	}
