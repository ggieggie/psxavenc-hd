...
void print_help(void)
{
	fprintf(stderr,
					...
					"    -s WxH           Rescale input file to fit within specified size (default 1280x768)\n"
...
int parse_args(settings_t *settings, int argc, char **argv)
{
	...
	while ((c = getopt(argc, argv, "?hqt:F:C:f:b:c:LR:i:a:s:IS:r:x:")) != -1)
	{
		...
		case 's':
		{
			...
			if (settings->video_width < 16 || settings->video_width > 1280)
			{
				fprintf(stderr, "Invalid video width: %d\n", settings->video_width);
				return -1;
			}
			if (settings->video_height < 16 || settings->video_height > 768)
			{
