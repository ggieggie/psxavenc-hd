...
void encode_file_str(settings_t *settings, FILE *output)
{
	...
	// [オリジナル] 実機のビットレート想定に基づいて「1フレームあたりのサイズ(frame_size)」を求めていた部分
	// --------------------------------------------------------------------------------------
	// settings->state_vid.frame_block_base_overflow = (75*settings->cd_speed)
	//     * video_sectors_per_block * settings->video_fps_den;
	// settings->state_vid.frame_block_overflow_den = interleave * settings->video_fps_num;
	// double frame_size = (double)settings->state_vid.frame_block_base_overflow
	//                    / (double)settings->state_vid.frame_block_overflow_den;
	// if (!settings->quiet) {
	...
	//
	// // frame_sizeとvideo_sectors_per_blockを用いたフレーム先読み数も計算していた
	// int frames_needed = (int) ceil((double)video_sectors_per_block / frame_size);
	// if (frames_needed < 2) frames_needed = 2;
	//
	// for (int j = 0; !settings->end_of_input
	//     || settings->state_vid.frame_data_offset < settings->state_vid.frame_max_size; j++) {
	...
	// ↑ この部分を削除・置き換えし、CD速によるサイズ制限を無視した実装へ修正する
	// ============================================================================
	// 【修正後】 大きめのフレームバッファを確保し、フレームごとにまるごと圧縮 → 2016バイト単位で複数セクタへ書き出す
	// ============================================================================

	// 大容量のバッファを一括確保(例: 10MB)：
	// これで大解像度でも余裕を持ってMDEC圧縮データを格納できるようにする
	const size_t BIG_FRAME_BUFFER_SIZE = 10 * 1024 * 1024; // 10MB
	settings->state_vid.frame_output = (uint8_t *)malloc(BIG_FRAME_BUFFER_SIZE);
	if (!settings->state_vid.frame_output)
	{
		fprintf(stderr, "Failed to allocate large frame buffer.\n");
		return;
	}

	// state_vid の関連メンバを初期化
	settings->state_vid.frame_index = 0;			 // 現在のフレーム番号
	settings->state_vid.frame_data_offset = 0; // 書き出した位置(バイトオフセット)
	// ここを追加して、frame_max_size = 実際に確保したバッファサイズ とする
	settings->state_vid.frame_max_size = BIG_FRAME_BUFFER_SIZE;
	settings->state_vid.frame_block_base_overflow = 0; // 使わない
	settings->state_vid.frame_block_overflow_num = 0;	 // 使わない
	settings->state_vid.frame_block_overflow_den = 1;	 // 使わない
	settings->state_vid.quant_scale_sum = 0;					 // 平均量子化スケール計算用
	// 'bytes_used' は encode_frame_bs() 内で「フレーム圧縮後の総バイト数」がセットされる
	...
	// メインループ：
	//   1) 音声/動画セクタを判断
	//   2) 動画セクタなら2016バイト刻みでフレームデータを切り出して書く
	//   3) フレームを書き切ったら次のフレームをエンコード
	//   4) 音声セクタなら、XAエンコード後に書き込み
	//   5) 入力が終わった & 最後のフレームも書き切ったら終了
	while (1)
	{
		// 一定量の音声/映像を先読み
		// ここでは音声があれば audio_samples_per_sector 分、動画は最低1フレーム(1以上)
		if (!ensure_av_data(settings, audio_samples_per_sector * settings->channels, 1))
		{
			// 入力切れかどうか確認
			if (settings->end_of_input && settings->state_vid.frame_data_offset >= settings->state_vid.bytes_used)
			{
				// 最後のフレームも書き終えたので終了
				break;
			}
		...
		if ((j % interleave) < video_sectors_per_block)
		{
			...
			// 「いま書き出し中のフレーム」はすでに全部書き終わっているか？
			//   → 新しくフレームをMDECエンコードしてbufferに置く
			if (settings->state_vid.frame_data_offset >= settings->state_vid.bytes_used)
			{
				// 次のフレームに進む
				settings->state_vid.frame_index++;
				// フレームをMDEC圧縮 (encode_frame_bs)
				// これが終わると、"settings->state_vid.frame_output" に
				// 圧縮済みデータが入り、 "bytes_used" に全体サイズが書かれる
				encode_frame_bs(settings->video_frames, settings);
				// 書き込みオフセットを0に戻す
				settings->state_vid.frame_data_offset = 0;
			}

			// 今のフレームで未書き出しのバイト数を算出
			size_t remaining = 0;
			if (settings->state_vid.bytes_used > settings->state_vid.frame_data_offset)
			{
				remaining = settings->state_vid.bytes_used - settings->state_vid.frame_data_offset;
			}
			// 1セクタにつき 2016 バイトを超えないようにする
			if (remaining > 2016)
			{
				remaining = 2016;
			}

			// STRのヘッダ領域(32バイト程度)をゼロクリア or 必要な初期化
			memset(buffer + 0x008, 0, 32);
			int headerOffset = 0x008;
			if (settings->format == FORMAT_STR2CD)
			{
				headerOffset = 0x018;
				memset(buffer + 0x018, 0, 32);
			}

			// このセクタが「フレーム内の何番目のチャンク」か計算( chunk_index )
			int chunk_index = (int)(settings->state_vid.frame_data_offset / 2016);
			// フレーム全体のチャンク数 (端数を切り上げ)
			int chunk_count = (settings->state_vid.bytes_used + 2015) / 2016;
			// ここで16ビット（65535）を超えないかチェック
			if (chunk_count > 65535)
			{
				fprintf(stderr, "Error: chunk_count (%d) exceeds 65535; frame is too large!\n", chunk_count);
				exit(1);
			}

			// [ヘッダ書き込み例]
			// STRv2 signature
			buffer[headerOffset + 0] = 0x60;
			buffer[headerOffset + 1] = 0x01;

			// Chunk type = MDEC data
			buffer[headerOffset + 2] = 0x01;
			buffer[headerOffset + 3] = 0x80;

			// チャンク番号 / 総チャンク数
			buffer[headerOffset + 4] = (uint8_t)(chunk_index & 0xFF);
			buffer[headerOffset + 5] = (uint8_t)((chunk_index >> 8) & 0xFF);
			buffer[headerOffset + 6] = (uint8_t)(chunk_count & 0xFF);
			buffer[headerOffset + 7] = (uint8_t)((chunk_count >> 8) & 0xFF);

			// フレームindex (32bit)
			int fi = settings->state_vid.frame_index;
			buffer[headerOffset + 8] = (uint8_t)(fi & 0xFF);
			buffer[headerOffset + 9] = (uint8_t)((fi >> 8) & 0xFF);
			buffer[headerOffset + 10] = (uint8_t)((fi >> 16) & 0xFF);
			buffer[headerOffset + 11] = (uint8_t)((fi >> 24) & 0xFF);

			// 幅・高さ
			buffer[headerOffset + 0x10] = (uint8_t)(settings->video_width & 0xFF);
			buffer[headerOffset + 0x11] = (uint8_t)((settings->video_width >> 8) & 0xFF);
			buffer[headerOffset + 0x12] = (uint8_t)(settings->video_height & 0xFF);
			buffer[headerOffset + 0x13] = (uint8_t)((settings->video_height >> 8) & 0xFF);

			// 残りデータをコピー (最大2016バイト)
			if (remaining > 0)
			{
				memcpy(
						buffer + headerOffset + 0x20,
						settings->state_vid.frame_output + settings->state_vid.frame_data_offset,
						remaining);
			}
			// 残りが少ない場合は末尾をゼロ埋め
			if (remaining < 2016)
			{
				memset(buffer + headerOffset + 0x20 + remaining, 0, 2016 - remaining);
			}

			// 書き込み済みオフセットを進める
			settings->state_vid.frame_data_offset += remaining;

			// STR2CD ならサブヘッダに時刻を入れて EDC 計算
			if (settings->format == FORMAT_STR2CD)
			{
				int t = j + 75 * 2; // 適当なMSF計算例
				buffer[0x00C] = (uint8_t)(((t / 75 / 60) % 10) | (((t / 75 / 60) / 10) << 4));
				buffer[0x00D] = (uint8_t)((((t / 75) % 60) % 10) | ((((t / 75) % 60) / 10) << 4));
				buffer[0x00E] = (uint8_t)(((t % 75) % 10) | (((t % 75) / 10) << 4));
		...
		else
		{
			...
			// 既存のロジックで XAエンコード
			int length = psx_audio_xa_encode(xa_settings, &audio_state,
																			 settings->audio_samples, samples_length, buffer);
			if (settings->end_of_input)
			{
				psx_audio_xa_encode_finalize(xa_settings, buffer, length);
			}
		...
		// 終了判定：
		// 入力が終わっており、かつ frame_data_offset >= bytes_used (フレームを書き終わってる) ならループ終了
		if (settings->end_of_input && (settings->state_vid.frame_data_offset >= settings->state_vid.bytes_used))
		{
			break;
		}
		...
		if (t)
		{
			// 平均量子化スケール
			double avg_q = 0.0;
			if (settings->state_vid.frame_index > 0)
			{
				avg_q = (double)settings->state_vid.quant_scale_sum / (double)settings->state_vid.frame_index;
			}
